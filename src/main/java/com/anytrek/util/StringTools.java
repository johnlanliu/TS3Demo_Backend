...
public class StringTools {
	...
	public static String toStringValue(byte b[], int ofs, int len) {
		if (b == null) {
			...
			try {
				return new String(b, ofs, len, "UTF-8");
			} catch (Throwable t) {
	 ...
	 **/
	public static <T> Number parseNumber(Object data, Class<?> numberClass,
			Number dft) {
		if (data == null) {
			return dft;
	 ...
	 **/
	public static double parseDouble(byte b[], int ofs, boolean isBigEndian,
			double dft) {
	 ...
	 **/
	public static float parseFloat(byte b[], int ofs, boolean isBigEndian,
			float dft) {
	...
	public static int parseInt(FilterNumber num, int dft) {
		if ((num != null) && num.supportsType(Integer.class)) {
			if (num.isHex()) {
				return (int) StringTools
						.parseHexLong(num.getValueString(), dft);
			} else {
				try {
	...
	public static class FilterNumber {
		...
		public FilterNumber(String val, Class<?> type) {
			...
			/* negative number */
			int ps, p = (!hex && (s < vlen) && (v.charAt(s) == '-')) ? (s + 1)
					: s;
			...
			if (hex) {
				...
				for (ps = p; (p < vlen)
						&& ("0123456789ABCDEF".indexOf(Character.toUpperCase(v
								.charAt(p))) >= 0);) {
					p++;
				}
			} else {
	 ...
	 *** @return The hex representation in the form of a StringBuffer
	 **/
	public static StringBuffer formatHexString(byte b[], int bOfs, int bLen,
			int blockLen, boolean showAscii, StringBuffer sb) {
		int headerLen = 0;
		if (b == null) {
			b = new byte[0];
		}
		...
		int bMaxNdx = ((bLen >= 0) && ((bi + bLen) <= b.length)) ? (bi + bLen)
				: b.length;
	 ...
	 *** @return The StringBuffer where the String hex representation is placed
	 **/
	public static StringBuffer toHexString(byte b[], int ofs, int len,
			StringBuffer sb) {
		if (sb == null) {
			sb = new StringBuffer();
		}
	...
	public static String toHexString(BigInteger val, int bitLen) {
		...
		if (bitCount > bitLen) {
			val = val.and(BigInteger.ONE.shiftLeft(bitLen).subtract(
					BigInteger.ONE));
		}
		...
		if (nybbleLen > hex.length()) {
			hex.insert(0,
					StringTools.replicateString("0", nybbleLen - hex.length()));
		}
		return hex.toString();
	...
	public static String toHexString(long val, int bitLen) {
		...
		// Print.logInfo("NybbleLen: " + nybbleLen + " : " + hex + " [" +
		// hex.length());
	 ...
	 **/
	public static StringBuffer toBinaryString(byte b[], int ofs, int len,
			StringBuffer sb) {
		if (sb == null) {
			sb = new StringBuffer();
		}
	 ...
	 **/
	public static String encodeArray(Object list[], char delim,
			boolean alwaysQuote) {
	 ...
	 *** @return The encoded String
	 **/
	public static String encodeArray(Object list[], int ofs, int max,
			char delim, boolean alwaysQuote) {
		StringBuffer sb = new StringBuffer();
		if (list != null) {
	 ...
	 *** @return The encoded String
	 **/
	public static String encodeArray(Object list, int ofs, int max, char delim,
			boolean alwaysQuote) {
		if (list == null) {
			// nothing to encode
			return "";
			...
			return StringTools.encodeArray((Object[]) list, ofs, max, delim,
					alwaysQuote);
			...
			if ((ofs <= 0) && (max != 0)) {
				...
				return alwaysQuote ? StringTools.quoteString(list.toString())
						: list.toString();
			} else {
	 ...
	 *** @return The encoded String
	 **/
	public static String encodeArray(java.util.List<Object> list, char delim,
			boolean alwaysQuote) {
		return StringTools.encodeArray(ListTools.toArray(list), delim,
				alwaysQuote);
	}
	...
	public static String encodeArray(java.util.List<Object> list) {
		return StringTools.encodeArray(ListTools.toArray(list), ARRAY_DELIM,
				true);
	}
	...
	public static String encodeArray(String list[], boolean alwaysQuote) {
		return StringTools.encodeArray((Object[]) list, ARRAY_DELIM,
				alwaysQuote);
	}
	...
	private static final int STRIP_INCLUDE = 0;
	private static final int STRIP_EXCLUDE = 1;
	...
	public static String replaceChars(String src, char ch, char repChar) {
		return StringTools.replaceChars(src, String.valueOf(ch),
	...
	public static String replaceKeys(String text, KeyValueMap keyMap) {
		return replaceKeys(text, keyMap, null, null/* keyStart */,
				null/* keyEnd */, null/* argDelim */, null/* dftDelim */);
	}
	...
	public static String replaceKeys(String text, KeyValueMap keyMap,
			ValueFilter filter) {
		return replaceKeys(text, keyMap, filter, null/* keyStart */,
				null/* keyEnd */, null/* argDelim */, null/* dftDelim */);
	}
	 ...
	 **/
	public static String replaceKeys(String text, KeyValueMap keyMap,
			ValueFilter filter, String keyStart, String keyEnd,
			String argDelim, String dftDelim) {
		...
		for (; (ks >= 0);) {
			...
			int ksi = ((ks > 0) && (repText.charAt(ks - 1) == KEY_START_ESC)) ? ks - 1
					: ks;
			...
			String kv = (keyMap != null) ? keyMap.getKeyValue(key, arg, null)
					: null;
			String fv = (kv != null) ? ((filter != null) ? filter
					.getFilteredValue(kv) : kv) : dftStr;
	...
	public static String padRight(String s, char padChar, int len) {
		if (s == null) {
			...
			return s
					+ StringTools.replicateString(String.valueOf(padChar), len
							- s.length());
		}
	}
	...
	public static class RegexIndex {
		...
		public int getStart() {
			return (this.matcher != null) ? this.matcher.start()
					: this.startPos;
		}
	...
	public static int compare(byte b1[], String s) {
		return StringTools.compare(b1, ((s != null) ? StringTools.getBytes(s)
				: null), -1);
	}
	...
	public static boolean compareEquals(byte b[], String s) {
		return StringTools.compareEquals(b,
				((s != null) ? StringTools.getBytes(s) : null), -1);
	}
...
//		System.out.println(isLong("035686904006599", true));
//		System.out.println(isBigInteger("035686904006599", true));
//		System.out.println(Long.parseLong("035686904006599"));
//	}
